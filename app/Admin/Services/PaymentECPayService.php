...
class PaymentECPayService
{
    ...
    public static function updateECPayOrder($data, $userId) {
        try {
            ...
            if ( $order_info['return_state'] !== 1 ) {
                ...
                Log::info('[updateECPayOrder Before] ' . $point);
                // 更新用戶訂單
                $user_info = self::getUserPayRecordByOrderId($data['MerchantTradeNo']);
                $where = [
                    ['id', $user_info['id']]
                ];
                UserPayRecord::query()->where($where)->update([
                    'user_payment_status' => 1,
                    'user_point_before'   => $point,
                    'user_point_after'    => $point + $user_info['user_point']
                ]);
                // 更新點數
                User::query()->where('id', $userId)->increment('point', $user_info['user_point']);
                // 更新點數後
                $point = User::query()->where('id', $userId)->value('point');
                Log::info('[updateECPayOrder After] ' . $point);
